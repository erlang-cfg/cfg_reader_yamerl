...
RELEASE_DIR  = $(CURDIR)/_build/default/rel/cfg_reader_yamerl
VERSION     := $(shell erl -noshell -eval 'io:format("~s", [case file:consult("$(CURDIR)/src/cfg_reader_yamerl.app.src") of {ok, [{_, _, Terms}]} -> proplists:get_value(vsn, Terms, "0.0.0"); _ -> "0.0.0" end]),erlang:halt().')
RELEASE_NAME = cfg_reader_yamerl-$(VERSION)
NAME_UPPER  := $(shell echo cfg_reader_yamerl | awk '{print toupper($$1)}')
...
compile: remove-beams
	...
	$(PRE) \
            ...
            $(REBAR) compile \
        $(POST)
	$(PRE) cp -r $(CURDIR)/_build/default/lib/cfg_reader_yamerl/ebin $(CURDIR)
...
shell: shell-compile
	...
	$(PRE) erl -pa `ls -d _build/default/lib/*/ebin` \
                   ...
                   -eval "begin application:load('cfg_reader_yamerl'), code:load_file('cfg_reader_yamerl'), case application:get_all_key('cfg_reader_yamerl') of {ok, K} -> lists:foreach(fun code:load_file/1, proplists:get_value(modules, K, [])); _ -> ok end end" \
                   +B
...
docker:
	$(PRE) docker build -t cfg_reader_yamerl ./ $(POST)
